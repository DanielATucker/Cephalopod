...
router.post('/newUser', function (req, res) {  
  ...
  Database(`MATCH (m: Main) CREATE (u: User), (T: TaskMaster), (CT: CalendarTrash), (TC: TaskCompleted), (CM: CalendarMaster), (u)-[r: link]->(m), (CT)-[s: TrashOf]->(CM), (T)-[t: link]->(u), (TC)-[l: link]->(T), (CM)-[CML: CalendarLink]->(u) SET u.name = '${username}', u.password = '${password}', u.privileges = 'user', u.loginHistory = '${JSON.stringify([{"createdTime": now}])}', u.createdTime = '${now}', T.name = 'TaskMaster', CT.name = 'CalendarTrash', TC.name = 'TaskCompleted', CM.name = 'CalendarMaster'`);
  res.end();
});
