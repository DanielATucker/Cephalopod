...
class Calendar1 extends React.Component {
  ...
  getCalendarData = () => {
    axios
      .get(`https://${process.env.host}:5001/calendar/get_events`)
      .then((returned) => {
  ...
  refreshData(action, eventItem) {
    switch (action) {
      case "update":
        axios
          .post(`https://${process.env.host}:5001/calendar/update_event`, {
      ...
      case "insert":
        axios
          .post(`https://${process.env.host}:5001/calendar/insert_event`, {
            eventData: eventItem.item,
            eventTitle: eventItem.item.label,
          })
          .then(function (result) {
      ...
      case "delete":
        axios
          .post(`https://${process.env.host}:5001/calendar/delete_event`, {
            eventData: eventItem.item,
            eventTitle: eventItem.item.label,
          })
          .then(function (result) {
